...
(async function build() {
  ...
  /**
   * Get the JS files from `./dist/htwooangularspfx/` directory and combine it into `bundle.js`
   * This will give support for angular v6 and all above versions.
   */
 
  try {
    let directoryPath = './dist/htwooangularspfx/';
    let files = await getFilesFromDirByType(directoryPath, 'js');
    files = files.map(file => directoryPath + file);
    await concat(files, './dist/htwooangularspfx/bundle.js');
  } catch (error) {
    console.log(error);
  }
})();
